...
namespace WebsiteMVC.Areas.AdminCP.Controllers
{
    ...
    {
        public HOUSEEntities1 db = new HOUSEEntities1();
        ...
        public JsonResult GetData(string sfromDate, string stoDate)
        {
            ...
            var thanhToan = db.TinTucs.Where(q => q.CreateTime >= fromDate && q.CreateTime <= toDate).ToList();
            ...
            var data = (from d in thanhToan
                        ...
                        select new BaoCaoThanhToan
                        {
                            SoBai = g.Count(),
                            Time = g.Key,
                            SoDuAn = g.GroupBy(q => q.IDTinTuc).Count(),
